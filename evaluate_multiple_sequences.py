...
for sequence in tqdm(kitti_sequences):
    ...
    sequence_path = "KITTI/data_odometry_color/dataset/sequences/" + sequence
    ...
    original = 'thirdparty/SuperPoint_SLAM/KeyFrameTrajectoryTUM.txt'
    now = datetime.now()
...
for sequence in tqdm(tartan_sequences):
    ...
    now = datetime.now()
    timestamp = f"Y{now.year}_M{now.month}_D{now.day}_H{now.hour}_M{now.minute}_S{now.second}"
    target = f'evaluations/{sequence.split("/")[-1]}_KeyFrameTrajectoryTUM_{timestamp}.txt'
